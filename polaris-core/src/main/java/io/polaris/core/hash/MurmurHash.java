...
public class MurmurHash {
	...
	// region Constants for 32 bit variant
	...
	// region Constants for 128 bit variant
	...
	public static int hash32(byte[] data, int offset, int length, int seed) {
		...
		for (int i = 0; i < nblocks; i++) {
			...
			final int k = Bytes.bytesToInt(data, i4, DEFAULT_ORDER);
			// mix functions
			hash = mix32(k, hash);
		}
	...
	public static long hash64(byte[] data, int length, int seed) {
		...
		for (int i = 0; i < nblocks; i++) {
			final int i8 = i << 3;
			long k = Bytes.bytesToLong(data, i8, DEFAULT_ORDER);
	...
	public static long[] hash128(byte[] data, int offset, int length, int seed) {
		...
		for (int i = 0; i < nblocks; i++) {
			...
			long k1 = Bytes.bytesToLong(data, i16, DEFAULT_ORDER);
			long k2 = Bytes.bytesToLong(data, i16 + 8, DEFAULT_ORDER);
