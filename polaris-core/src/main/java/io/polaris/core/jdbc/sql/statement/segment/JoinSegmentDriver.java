 ...
 */
public class JoinSegmentDriver<O extends SelectStatement<O>, J extends JoinSegment<O, J>> extends BaseSegment<JoinSegmentDriver<O, J>> {
	private final O owner;
	private final JoinBuilder<O, J> builder;
	private final Consumer<J> consumer;
	private TextNode conj = SqlNodes.JOIN;

	public JoinSegmentDriver(O owner, Consumer<J> consumer, JoinBuilder<O, J> builder) {
		this.owner = owner;
	...
	public JoinSegmentDriver<O, J> inner() {
		this.conj = SqlNodes.INNER_JOIN;
		return getThis();
	}

	public JoinSegmentDriver<O, J> left() {
		this.conj = SqlNodes.LEFT_JOIN;
		return getThis();
	}

	public JoinSegmentDriver<O, J> right() {
		this.conj = SqlNodes.RIGHT_JOIN;
		return getThis();
	}

	public JoinSegmentDriver<O, J> outer() {
