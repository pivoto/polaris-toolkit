...
public class PackageChangerRunner {
	...
	public static void change(InputStream in) throws DocumentException, IOException {
		...
		for (Element change : changes) {
			...
			String includeFileName = change.elementTextTrim("include-filename");
			if (includeFileName != null && includeFileName.length() > 0) {
				pc.setIncludeFileName(Boolean.valueOf(includeFileName));
			}
			...
			List<Element> namePatterns = change.elements("name-pattern");
			for (Element element : namePatterns) {
				String namePattern = element.getTextTrim();
