...
import io.polaris.toolkit.core.constant.Constants;
...
public class RSA {
	...
	public static String decrypt(PublicKey publicKey, String cipherText)
		...
		return new String(plainBytes, Constants.UTF_8);
	}
	...
	public static String encrypt(PrivateKey privateKey, String plainText)
		...
		byte[] encryptedBytes = cipher.doFinal(plainText.getBytes(Constants.UTF_8));
	...
	public static PublicKey getPublicKeyByX509(String x509File) {
		...
		try (InputStream in = IOUtils.getInputStream(x509File)) {
	...
	public static PublicKey getPublicKeyByFile(String file) {
		...
		try (InputStream in = IOUtils.getInputStream(file)) {
			byte[] publicKeyBytes = IOUtils.toBytes(in, 64);
			return toPublicKey(publicKeyBytes);
		} catch (IOException e) {
			throw new IllegalArgumentException("Failed to get public key", e);
		}
